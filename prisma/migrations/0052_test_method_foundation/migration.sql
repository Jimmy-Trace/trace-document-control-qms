...
CREATE OR REPLACE FUNCTION reject_laboratory_method_version_mutation() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'LaboratoryMethodVersion is append-only'; END; $$ LANGUAGE plpgsql;
CREATE TRIGGER "LaboratoryMethodVersion_append_only" BEFORE UPDATE OR DELETE ON "LaboratoryMethodVersion" FOR EACH ROW EXECUTE FUNCTION reject_laboratory_method_version_mutation();
