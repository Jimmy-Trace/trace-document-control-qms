...
export function TrainingManagementWorkspace({ canManage, today }: { canManage: boolean; today: string }) {
  ...
  return <section className="workspace-section" aria-labelledby="training-heading">
    ...
    <nav className="module-subnav" aria-label="Training management sections">{sections.map((item) => <button key={item.id} type="button" className={section === item.id ? "active" : ""} onClick={() => setSection(item.id)}><strong>{item.label}</strong><span>{item.description}</span></button>)}</nav>
