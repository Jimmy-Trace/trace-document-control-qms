...
export function QualityEventGovernedActions({event,canManage,onChanged}:{event:EventRef;canManage:boolean;onChanged:()=>Promise<void>}){
  ...
  async function load(){
    ...
    const ib=await json(i);const cb=await json(c);
  ...
  useEffect(()=>{let active=true;Promise.all([
    fetch(`/api/quality/events/${event.id}/investigations`,{credentials:"same-origin"}),
    fetch(`/api/quality/events/${event.id}/capa`,{credentials:"same-origin"}),
  ]).then(async([i,c])=>({i,c,ib:await json(i),cb:await json(c)})).then(({i,c,ib,cb})=>{if(!active)return;if(i.ok)setInvestigations(ib?.data??[]);if(c.ok)setCapas(cb?.data??[]);});return()=>{active=false;};},[event.id]);
  ...
  return <div>
    <h3>Investigation, CAPA, and controlled closure — {event.eventNumber}</h3>
    {error&&<p role="alert">{error}</p>}{notice&&<p role="status">{notice}</p>}
    <h4>Investigation evidence</h4>
    {investigations.length?<ul>{investigations.map(i=><li key={i.id}>{i.rootCauseMethod}: {i.rootCause} · risk {i.riskScore} ({i.riskLikelihood}×{i.riskImpact})</li>)}</ul>:<p>No investigation evidence recorded.</p>}
    {canManage&&event.status!=="OPEN"&&event.status!=="CLOSED"&&<form onSubmit={addInvestigation}>
      <label>Findings<textarea required value={findings} onChange={e=>setFindings(e.target.value)} maxLength={10000}/></label>
      <label>Affected scope<textarea required value={affectedScope} onChange={e=>setAffectedScope(e.target.value)} maxLength={5000}/></label>
      <label>Evidence summary<textarea value={evidenceSummary} onChange={e=>setEvidenceSummary(e.target.value)} maxLength={5000}/></label>
      <label>Root-cause method<select value={rootCauseMethod} onChange={e=>setRootCauseMethod(e.target.value)}><option value="FIVE_WHYS">Five Whys</option><option value="FISHBONE">Fishbone</option><option value="FAULT_TREE">Fault tree</option><option value="OTHER">Other</option></select></label>
      <label>Root cause<textarea required value={rootCause} onChange={e=>setRootCause(e.target.value)} maxLength={5000}/></label>
      <label>Likelihood (1–5)<input type="number" min={1} max={5} value={riskLikelihood} onChange={e=>setRiskLikelihood(Number(e.target.value))}/></label>
      <label>Impact (1–5)<input type="number" min={1} max={5} value={riskImpact} onChange={e=>setRiskImpact(Number(e.target.value))}/></label>
      <button disabled={busy}>Record investigation</button>
    </form>}
    <h4>CAPA actions</h4>
    {capas.length?<ul>{capas.map(c=><li key={c.id}><strong>{c.actionType}</strong> — {c.description} · {c.status} · due {c.dueAt.slice(0,10)}
      {canManage&&c.status==="OPEN"&&<div><label>Completion evidence<textarea value={completionEvidence[c.id]??""} onChange={e=>setCompletionEvidence(v=>({...v,[c.id]:e.target.value}))}/></label><button type="button" disabled={busy||!(completionEvidence[c.id]??"").trim()} onClick={()=>void complete(c)}>Complete CAPA</button></div>}
      {canManage&&c.status==="COMPLETED"&&<div><label>Effectiveness result<select value={verificationResult[c.id]??"PASS"} onChange={e=>setVerificationResult(v=>({...v,[c.id]:e.target.value as "PASS"|"FAIL"}))}><option value="PASS">Pass</option><option value="FAIL">Fail</option></select></label><label>Effectiveness evidence<textarea value={verificationEvidence[c.id]??""} onChange={e=>setVerificationEvidence(v=>({...v,[c.id]:e.target.value}))}/></label><button type="button" disabled={busy||!(verificationEvidence[c.id]??"").trim()} onClick={()=>void verify(c)}>Record effectiveness</button></div>}
    </li>)}</ul>:<p>No CAPA actions recorded.</p>}
    {canManage&&event.status!=="OPEN"&&event.status!=="CLOSED"&&<form onSubmit={addCapa}><label>Action type<select value={actionType} onChange={e=>setActionType(e.target.value as "CORRECTIVE"|"PREVENTIVE")}><option value="CORRECTIVE">Corrective</option><option value="PREVENTIVE">Preventive</option></select></label><label>Description<textarea required value={description} onChange={e=>setDescription(e.target.value)} maxLength={5000}/></label><label>Owner user ID<input required value={ownerUserId} onChange={e=>setOwnerUserId(e.target.value)}/></label><label>Due date<input required type="date" value={dueAt} onChange={e=>setDueAt(e.target.value)}/></label><button disabled={busy}>Create CAPA</button></form>}
    {canManage&&event.status==="VERIFICATION"&&<form onSubmit={close}><h4>Controlled final closure</h4><p>Closure is accepted only when investigation exists, all CAPAs are complete, and each CAPA has a passing effectiveness check. Password reauthentication and electronic-signature evidence are required.</p><label>Closure reason<textarea required value={closureReason} onChange={e=>setClosureReason(e.target.value)} maxLength={5000}/></label><label>Password<input required type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)}/></label><label><input type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/> I confirm this quality event is complete, required CAPA actions have been implemented and verified effective, and the event may be closed.</label><button disabled={busy||!confirmed||!closureReason.trim()||!password}>Close with electronic signature</button></form>}
  </div>;
}
