...
export function EquipmentComplianceWorkspace({canManage}:{canManage:boolean}){
 ...
 return <section className="card form-stack"><h3>Equipment compliance controls</h3><p>Review system-created overdue compliance holds, record retrospective impact assessment, and clear holds only after current calibration or maintenance evidence supports release.</p>{error&&<p className="status-error">{error}</p>}<label>Equipment<select value={selected} onChange={e=>setSelected(e.target.value)} disabled={!equipment.length}>{equipment.length?equipment.map(item=><option key={item.id} value={item.id}>{item.equipmentNumber} — {item.name}</option>):<option value="">No equipment registered</option>}</select></label>{selected&&<EquipmentCompliancePanel key={selected} equipmentId={selected} canManage={canManage} onChanged={refresh}/>}</section>;
}
