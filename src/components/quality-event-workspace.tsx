...
export function QualityEventWorkspace({canManage,today}:{canManage:boolean;today:string}){
  ...
  useEffect(()=>{
    ...
    Promise.all([
      fetch("/api/quality/events",{credentials:"same-origin"}),
      fetch("/api/quality/events/analytics?months=12",{credentials:"same-origin"}),
    ]).then(async([eventsResponse,analyticsResponse])=>({eventsResponse,analyticsResponse,eventsBody:await eventsResponse.json().catch(()=>null),analyticsBody:await analyticsResponse.json().catch(()=>null)})).then(({eventsResponse,analyticsResponse,eventsBody,analyticsBody})=>{
      if(!active)return;
      if(eventsResponse.ok)setEvents(eventsBody?.data??[]);else setError(eventsBody?.error??"Unable to load quality events");
      if(analyticsResponse.ok)setAnalytics(analyticsBody?.data??null);
    });
  ...
  return <section className="panel" aria-labelledby="quality-event-workspace-heading">
    ...
    {canManage&&selected&&selected.status!=="CLOSED"&&<div><h3>Governed lifecycle action — {selected.eventNumber}</h3><label>Required reason<textarea value={reason} onChange={event=>setReason(event.target.value)} maxLength={1000}/></label>{nextStatus[selected.status]&&<button type="button" disabled={busy||!reason.trim()} onClick={()=>void update({status:nextStatus[selected.status]})}>Advance to {nextStatus[selected.status]}</button>}<form onSubmit={saveMetadata}><label>Owner user ID<input value={ownerUserId} onChange={event=>setOwnerUserId(event.target.value)} placeholder="Active same-tenant user UUID"/></label><label>Due date<input type="date" value={dueAt} onChange={event=>setDueAt(event.target.value)}/></label><button type="submit" disabled={busy||!reason.trim()||(!ownerUserId&&! dueAt)}>Update assignment / due date</button></form></div>}
    {selected&&<QualityEventGovernedActions event={{id:selected.id,eventNumber:selected.eventNumber,status:selected.status}} canManage={canManage} onChanged={load}/>} 
  </section>;
}
