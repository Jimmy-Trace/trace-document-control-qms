...
export function ApprovalOperations() {
  ...
  async function loadQueue() {
    ...
    if (!response.ok) {
      ...
      setError(
        response.status === 403
          ? "Approval assignment requires review-management access."
          : body?.error || "Unable to load approval assignments.",
      );
      return;
    }
  ...
  return (
    <>
      <button
        type="button"
        style={{ position: "fixed", right: 210, bottom: 24, zIndex: 80 }}
        onClick={openPanel}
      >
      ...
      {open && (
        <div className="modal-backdrop" role="presentation">
          <div className="modal detail-modal" role="dialog" aria-modal="true" aria-labelledby="approval-operations-title">
            <div className="modal-head">
              <div>
                <h2 id="approval-operations-title">Approval assignments</h2>
                <p>Assign an eligible final approver before electronic signature.</p>
              </div>
              <button aria-label="Close approval assignments" onClick={() => setOpen(false)}>×</button>
            </div>
            <div className="detail-body">
              ...
              {queue && queue.tasks.length === 0 && (
                <div className="permission-state">
                  <strong>No active approval tasks</strong>
                  <span>Documents appear here after all required review stages are completed.</span>
                </div>
              )}
              {queue && queue.tasks.length > 0 && (
                    ...
                    <select value={selectedTaskId} onChange={(event) => setSelectedTaskId(event.target.value)} required>
                      {queue.tasks.map((task) => (
                        <option key={task.id} value={task.id}>
                          {task.documentNumber} · v{task.revisionLabel} · {task.title}
                        </option>
                      ))}
                    </select>
                  </label>
                  {selectedTask?.assigneeUserId && (
                    <small>This task already has an approver and may be reassigned with a controlled reason.</small>
                  )}
                  <label>
                    Final approver
                    <select name="approverUserId" required defaultValue="">
                      <option value="" disabled>Select an eligible approver</option>
                      {queue.approvers.map((approver) => (
