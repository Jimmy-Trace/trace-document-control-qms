...
import { EquipmentRecallService,EquipmentRecallValidationError,type EquipmentRecallRecord,type EquipmentRecallStore } from "./recall";
...
const store=():EquipmentRecallStore=>({list:vi.fn(async()=>[]),open:vi.fn(async input:Parameters<EquipmentRecallStore["open"]>[0]):Promise<EquipmentRecallRecord>=>({id:"00000000-0000-0000-0000-000000000004",organizationId,equipmentId:input.equipmentId,reason:input.reason,scopeSummary:input.scopeSummary,openedByUserId:input.actorUserId,openedAt:new Date(),status:"OPEN",closedByUserId:null,closedAt:null,closureReason:null})),close:vi.fn(async input:Parameters<EquipmentRecallStore["close"]>[0]):Promise<EquipmentRecallRecord>=>({id:input.recallId,organizationId,equipmentId:input.equipmentId,reason:"Recall",scopeSummary:"Scope",openedByUserId:userId,openedAt:new Date(),status:"CLOSED",closedByUserId:input.actorUserId,closedAt:new Date(),closureReason:input.reason}))});
