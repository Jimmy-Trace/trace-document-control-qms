...
import { ReportingError,ReportingService,reportSourceKeys,validateReportingParameters } from "./reporting";
...
describe("governed reporting boundary",()=>{
  ...
  it("rejects arbitrary parameters in the foundation slice",()=>{
    expect(validateReportingParameters({})).toEqual({});
    expect(()=>validateReportingParameters({sql:"select * from User"})).toThrow(ReportingError);
  });
