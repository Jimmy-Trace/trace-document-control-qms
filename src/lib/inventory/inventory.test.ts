...
const store=():InventoryStore=>({listMaterials:vi.fn().mockResolvedValue([]),createMaterial:vi.fn(),listLots:vi.fn().mockResolvedValue([]),receiveLot:vi.fn(),transitionLot:vi.fn(),listBalances:vi.fn().mockResolvedValue([]),transact:vi.fn(),transfer:vi.fn()});
