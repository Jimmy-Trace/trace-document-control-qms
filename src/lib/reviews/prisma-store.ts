...
export class PrismaReviewStore implements ReviewStore {
  async listDue(organizationId: string, through: Date) {
    const rows = await db.documentReviewTask.findMany({
      where: {
        organizationId,
        status: "PENDING",
        dueAt: { lte: through },
      ...
      select: {
        ...
        documentVersion: { select: { revisionLabel: true } },
      },
      orderBy: { dueAt: "asc" },
    });
    return rows
      .filter((row) => row.document.currentVersionId === row.documentVersionId)
      .map((row) => ({
        id: row.id,
  ...
  async remind(input: Parameters<ReviewStore["remind"]>[0]) {
    return db.$transaction(async (transaction) => {
      const task = await transaction.documentReviewTask.findFirst({
        where: {
          organizationId: input.organizationId,
          id: input.taskId,
          status: "PENDING",
          dueAt: { gt: input.occurredAt },
          documentVersion: { status: "EFFECTIVE" },
        },
        select: {
          id: true,
      ...
      if (!task || task.document.currentVersionId !== task.documentVersionId)
        return false;
      ...
      await db.$transaction(async (transaction) => {
        ...
        await transaction.auditEvent.create({
          data: {
            ...
            entityId: task.id,
            occurredAt: input.occurredAt,
            metadata: { level: input.level, eventKey: input.eventKey },
          },
        });
      });
      return true;
    ...
    return db.$transaction(async (transaction) => {
      ...
      await transaction.auditEvent.create({
        data: {
          ...
          action: "DOCUMENT_PERIODIC_REVIEW_COMPLETED",
          entityType: "DocumentReviewTask",
