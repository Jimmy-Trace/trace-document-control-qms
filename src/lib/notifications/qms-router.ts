...
export class QmsNotificationRouter {
  ...
  async publish(input:{organizationId:string;topicKey:string;eventKey:string;payload:unknown}){
    const eventKey=input.eventKey.trim();
    if(!input.organizationId||!input.topicKey.trim()||!eventKey)throw new QmsNotificationRoutingError("Organization, topic, and event key are required");
    return db.$transaction(async tx=>{
      const rule=(await tx.$queryRaw<Array<{topicPolicyId:string;criticality:Criticality;defaultInApp:boolean;defaultEmail:boolean;recipientPermissionKey:string;templateKey:string}>>(Prisma.sql`
        ...
        WHERE p."organizationId"=${input.organizationId}::uuid AND p."topicKey"=${input.topicKey}`))[0];
      if(!rule)throw new QmsNotificationRoutingError("Active notification routing rule not found");
      const recipients=await tx.$queryRaw<Array<{userId:string;inAppEnabled:boolean|null;emailEnabled:boolean|null}>>(Prisma.sql`
