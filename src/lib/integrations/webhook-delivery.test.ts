...
describe("webhook delivery boundary",()=>{
  it("derives deterministic per-subscription secrets without persisting plaintext",()=>{
    const source: NodeJS.ProcessEnv={NODE_ENV:"test",WEBHOOK_SIGNING_MASTER_SECRET:"0123456789abcdef0123456789abcdef"};
  ...
  it("requires a sufficiently strong webhook signing master secret",()=>{
    const source: NodeJS.ProcessEnv={NODE_ENV:"test",WEBHOOK_SIGNING_MASTER_SECRET:"short"};
    expect(()=>deriveWebhookSigningSecret("subscription",1,source)).toThrow("at least 32 characters");
  });
