...
const store=():QualityCapaStore=>({list:vi.fn(async()=>[]),create:vi.fn(async()=>action),complete:vi.fn(async()=>({...action,status:"COMPLETED",completedAt:new Date(),completionEvidence:"Implemented"})),verify:vi.fn(async()=>check)});
