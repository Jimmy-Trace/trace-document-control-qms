...
const record:QualityInvestigationRecord={id:"00000000-0000-0000-0000-000000000004",organizationId,eventId,sequence:1,findings:"Findings",affectedScope:"Scope",evidenceSummary:null,evidenceFileId:null,rootCauseMethod:"FIVE_WHYS",rootCause:"Cause",riskLikelihood:3,riskImpact:4,riskScore:12,investigatorUserId:userId,createdAt:new Date()};
...
describe("QualityInvestigationService",()=>{
  ...
  it("normalizes evidence before persistence",async()=>{const s=store();const service=new QualityInvestigationService(s);await service.create(context(["quality_event.manage"]),{organizationId,eventId,findings:"  Findings  ",affectedScope:"  Scope  ",evidenceSummary:"  Evidence  ",rootCauseMethod:"FISHBONE",rootCause:"  Cause  ",riskLikelihood:3,riskImpact:4});expect(s.create).toHaveBeenCalledWith(expect.objectContaining({findings:"Findings",affectedScope:"Scope",evidenceSummary:"Evidence",evidenceFileId:null,rootCause:"Cause",investigatorUserId:userId}));});
});
