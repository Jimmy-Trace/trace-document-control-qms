...
export type QualityInvestigationRecord={id:string;organizationId:string;eventId:string;sequence:number;findings:string;affectedScope:string;evidenceSummary:string|null;rootCauseMethod:RootCauseMethod;rootCause:string;riskLikelihood:number;riskImpact:number;riskScore:number;investigatorUserId:string;createdAt:Date};

export interface QualityInvestigationStore {
  list(organizationId:string,eventId:string):Promise<QualityInvestigationRecord[]>;
  create(input:{organizationId:string;eventId:string;findings:string;affectedScope:string;evidenceSummary:string|null;rootCauseMethod:RootCauseMethod;rootCause:string;riskLikelihood:number;riskImpact:number;investigatorUserId:string}):Promise<QualityInvestigationRecord>;
}
...
export class QualityInvestigationService {
  ...
  create(context:AuthorizationContext,input:{organizationId:string;eventId:string;findings:string;affectedScope:string;evidenceSummary?:string|null;rootCauseMethod:RootCauseMethod;rootCause:string;riskLikelihood:number;riskImpact:number}){
    ...
    return this.store.create({organizationId:input.organizationId,eventId:input.eventId,findings,affectedScope,evidenceSummary,rootCauseMethod:input.rootCauseMethod,rootCause,riskLikelihood:input.riskLikelihood,riskImpact:input.riskImpact,investigatorUserId:context.userId});
  }
}
