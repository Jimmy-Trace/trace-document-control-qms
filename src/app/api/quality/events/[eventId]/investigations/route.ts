...
const createSchema=z.object({findings:z.string().max(10000),affectedScope:z.string().max(5000),evidenceSummary:z.string().max(5000).nullish(),evidenceFileId:z.string().uuid().nullish(),rootCauseMethod:z.enum(["FIVE_WHYS","FISHBONE","FAULT_TREE","OTHER"]),rootCause:z.string().max(5000),riskLikelihood:z.number().int().min(1).max(5),riskImpact:z.number().int().min(1).max(5)});
