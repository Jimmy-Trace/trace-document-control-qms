...
const schema=z.discriminatedUnion("operation",[
  z.object({operation:z.literal("COMPLETE"),completionEvidence:z.string().max(5000)}),
  z.object({operation:z.literal("VERIFY"),result:z.enum(["PASS","FAIL"]),evidence:z.string().max(5000)}),
]);

export async function POST(request:NextRequest,{params}:{params:Promise<{eventId:string;capaActionId:string}>}){try{const context=await authenticateRequest(request);const {eventId,capaActionId}=await params;const input=schema.parse(await request.json());const data=input.operation==="COMPLETE"?await service.complete(context,{organizationId:context.organizationId,eventId,capaActionId,completionEvidence:input.completionEvidence}):await service.verify(context,{organizationId:context.organizationId,eventId,capaActionId,result:input.result,evidence:input.evidence});return NextResponse.json({data});}catch(error){return respond(error);}}
