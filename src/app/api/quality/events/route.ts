...
export async function POST(request:NextRequest){
  try{const context=await authenticateRequest(request);const input=createSchema.parse(await request.json());return NextResponse.json({data:await service.createEvent(context,{organizationId:context.organizationId,...input,source:"MANUAL"})},{status:201});}catch(error){return respond(error);}
}
